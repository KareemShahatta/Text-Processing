...
public class InPlaceReverse
{
    ...
    {
        int limitIndex = s.length - 1;

        for(int lockIndex = 0 ; lockIndex < limitIndex; lockIndex ++)
        {
            char lastElement = s[limitIndex];

            for(int shiftIndex = limitIndex ; shiftIndex > lockIndex ; shiftIndex--)
            {
                s[shiftIndex] = s[shiftIndex -1];
            }

            s[lockIndex] = lastElement;
        }
    }
